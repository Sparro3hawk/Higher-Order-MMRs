...
def find_simulation_parameters(sim_results, run, bin_file):
    ...
    random_pnum = int(sim_results['pnum'].iloc[run])
    ...
    sim_read_in_for_masses = rebound.Simulation(bin_file)
    random_pl_masses = np.array([])
    for planet_idx in range(1, random_pnum+1): # avoids reading in star mass
        random_pl_masses = np.append(random_pl_masses, sim_read_in_for_masses.particles[planet_idx].m) 
    del sim_read_in_for_masses
    ...
    # We can compute other parameters from these inputs, but its better to be careful due to version differences etc. 
    taua = sim_results['taua'].iloc[run]
    taue = sim_results['taue'].iloc[run]
    K = sim_results['K'].iloc[run]
    K_outermost = sim_results['K_outermost'].iloc[run]
    h_0 = sim_results['h'].iloc[run]
    h_outermost = sim_results['h_outermost'].iloc[run]
    assert round(K, 6) == round(taua/taue, 6) 
    itime = sim_results['itime'].iloc[run]
    assert minimum_itime<=itime<=maximum_itime
    
    # 2.6.3: Return specific values. 
    return random_pnum, random_pl_masses, random_st_mass, random_pl_orbsmaxes, random_pl_orbeccens, random_pl_orbincs, random_pl_long_ascs, random_pl_arg_pericenters, random_pl_mean_anoms, disk_inner_edge, disk_inner_edge_width, K, sd_0, alpha, beta, minimum_itime, maximum_itime, timestep, taua, taue, K, K_outermost, h_0, h_outermost, itime 
...
def plot_save_print(times, a, omega, Omega, lam, per, e, random_pnum, version, date, txt_file, planet_count_max = 7):
    ...
    if cutoff<1:
        ...
        cutoff = 1
    # 4.0.0.1: Save parameters for all planets that we do not need to combine.
    np.save('save/Migration Reruns '+ date+"/"+version+'_times.npy', times)
    np.save('save/Migration Reruns '+ date+"/"+version+'_as.npy', a)
    np.save('save/Migration Reruns '+ date+"/"+version+'_omegas.npy', omega)
    np.save('save/Migration Reruns '+ date+"/"+version+'_Omegas.npy', Omega)
    np.save('save/Migration Reruns '+ date+"/"+version+'_lams.npy', lam)
    np.save('save/Migration Reruns '+ date+"/"+version+'_pers.npy', per)
    np.save('save/Migration Reruns '+ date+"/"+version+'_es.npy', e)
    ...
    plt.legend()
    plt.savefig('plots/pdfs/Migration Reruns '+date+"/"+version+'a.pdf')
    plt.savefig('plots/jpgs/Migration Reruns '+date+"/"+version+'a.jpg')

    # 4.0.2: Plot, save, and print the semi-major axis ratio for each adjacent planet pair.
    a_ratio = np.zeros((random_pnum-1, len(times)))
    plt.subplots(1,1,figsize=(10,3))
    plt.subplots_adjust(left=0.1, right=0.95, top=0.95, bottom=0.15)
    for i in range(planet_count_max-1):
        ...
        # 4.0.2.3: Print the final value of each semi-major axis ratio averaged after the cutoff.
        print("\t* Final Semi-Major Axis Ratio of Planet Pair "+str(i)+str(i+1)+": "+str(np.nanmean(a[i+1,-cutoff:]/a[i,-cutoff:])))
        # 4.0.2.4: Append the semi-major axis ratio of each planet pair. 
        a_ratio[i,:] = a[i+1,:]/a[i,:]
    plt.ylabel('a ratio')
    plt.legend()
    plt.xlabel('Time [kyr]')
    plt.savefig('plots/pdfs/Migration Reruns '+date+"/"+version+'a_ratio.pdf')
    plt.savefig('plots/jpgs/Migration Reruns '+date+"/"+version+'a_ratio.jpg')
    # 4.0.2.5: Save the semi-major axis ratios.
    np.save('save/Migration Reruns '+ date+"/"+version+'_a_ratios.npy', a_ratio)
    ...
    plt.legend()
    plt.xlabel('Time [kyr]')
    plt.savefig('plots/pdfs/Migration Reruns '+date+"/"+version+'P.pdf')
    plt.savefig('plots/jpgs/Migration Reruns '+date+"/"+version+'P.jpg')

    # 4.0.4: Plot and save the period ratio for each adjacent planet pair.
    p_ratio = np.zeros((random_pnum-1, len(times)))
    plt.subplots(1,1,figsize=(10,3))
    plt.subplots_adjust(left=0.1, right=0.95, top=0.95, bottom=0.15)
    for i in range(planet_count_max-1):
        ...
        # 4.0.4.4: Append the semi-major axis ratio of each planet pair. 
        p_ratio[i,:] = per[i+1,:]/per[i,:]
    plt.ylabel('Period Ratio')
    plt.legend()
    plt.xlabel('Time [kyr]')
    plt.savefig('plots/pdfs/Migration Reruns '+date+"/"+version+'P_ratio.pdf')
    plt.savefig('plots/jpgs/Migration Reruns '+date+"/"+version+'P_ratio.jpg')
    # 4.0.4.5: Save the period ratios.
    np.save('save/Migration Reruns '+ date+"/"+version+'_p_ratios.npy', p_ratio)
    ...
    plt.xlabel('Time [kyr]')
    plt.legend()
    plt.savefig('plots/pdfs/Migration Reruns '+date+"/"+version+'e.pdf')
    plt.savefig('plots/jpgs/Migration Reruns '+date+"/"+version+'e.jpg')
    ...
    delta_set = np.zeros((random_pnum-1, len(times)))
    plt.subplots(1,1,figsize=(10,3))
    plt.subplots_adjust(left=0.1, right=0.95, top=0.95, bottom=0.15)
    for i in range(planet_count_max-1):
        ...
        # 4.0.6.9: Append the delta of each planet pair. 
        delta_set[i,:] = smallest_delta
    plt.ylabel(r'$\Delta$')
    plt.legend()
    plt.xlabel('Time [kyr]')
    plt.savefig('plots/pdfs/Migration Reruns '+date+"/"+version+'delta.pdf')
    plt.savefig('plots/jpgs/Migration Reruns '+date+"/"+version+'delta.jpg')
    # 4.0.6.10: Save the semi-major axis ratios.
    np.save('save/Migration Reruns '+ date+"/"+version+'_deltas.npy', delta_set)

    # 4.0.7: Plot, save, and print the two body resonant angles of each adjacent planet pair.
    two_body_resonant_angle = np.zeros((random_pnum-1, len(times)))
    plt.subplots(1,1,figsize=(10,3))
    plt.subplots_adjust(left=0.1, right=0.95, top=0.95, bottom=0.15)
    for i in range(planet_count_max-1):
        ...
        # 4.0.7.11: Append the two body resonant angle of each planet pair. 
        two_body_resonant_angle[i,:] = phi_hat
    plt.xlabel('Time [kyr]')
    plt.ylabel(r'2-Body Critical Angle $\phi$ [deg]')
    plt.ylim(0,360)
    plt.legend(loc = 'lower right')
    plt.savefig('plots/pdfs/Migration Reruns '+date+"/"+version+'2body_angle.pdf')
    plt.savefig('plots/jpgs/Migration Reruns '+date+"/"+version+'2body_angle.jpg')
    # 4.0.7.12: Save the two body resonant angles. 
    np.save('save/Migration Reruns '+ date+"/"+version+'_two_body_resonant_angles.npy', two_body_resonant_angle)

    # 4.0.8: Plot and save the three body resonant angles of each adjacent planet pair.
    three_body_resonant_angle = np.zeros((random_pnum-1, len(times)))
    plt.subplots(1,1,figsize=(10,3))
    plt.subplots_adjust(left=0.1, right=0.95, top=0.95, bottom=0.15)
    for i in range(planet_count_max-2):
        ...
        # 4.0.8.11: Append the three body resonant angle of each planet pair. 
        three_body_resonant_angle[i,:] = phi_hat
    plt.xlabel('Time [kyr]')
    plt.ylabel(r'3-Body Critical Angle $\phi$ [deg]')
    plt.ylim(0,360)
    plt.legend(loc = 'lower right')
    plt.savefig('plots/pdfs/Migration Reruns '+date+"/"+version+'3body_angle.pdf')
    plt.savefig('plots/jpgs/Migration Reruns '+date+"/"+version+'3body_angle.jpg')
    # 4.0.8.12: Save the three body resonant angles. 
    np.save('save/Migration Reruns '+ date+"/"+version+'_three_body_resonant_angles.npy', three_body_resonant_angle)
    plt.close('all')
...
def run_migration_edge(index, txt_file, simulation_parameters, parameter_method, date, planet_count_max = 7, integration_time = "3taua", n_step=10000):
    random_pnum, random_pl_masses, random_st_mass, random_pl_orbsmaxes, random_pl_orbeccens, random_pl_orbincs, random_pl_long_ascs, random_pl_arg_pericenters, random_pl_mean_anoms, inner_edge, inner_edge_width, K, sd_0, alpha, beta, minimum_itime, maximum_itime, timestep, tau_a, tau_e, K, K_outermost, h_0, h_outermost, itime  = simulation_parameters
    ...
    sim.save_to_file('save/Migration Reruns '+ date+"/"+version+str(index)+'.bin')
...
# Stage 6: Run our particularly interesting systems. 
systems_to_rerun = pd.read_csv('Exoplanet Datasets/second_order_systems_to_rerun.csv')


# Stage 6: Run the code by defining a runit function with a variable number of simulations.
def runit(date, systems_to_rerun):
        # 6.1: Remove previous file versions with the same name if present.
        if os.path.exists("save/Migration Reruns "+ date+"/") == True:
            os.rmdir("save/Migration Reruns "+ date+"/")
        if os.path.exists("plots/pdfs/Migration Reruns "+ date+"/") == True:
            os.rmdir("plots/pdfs/Migration Reruns "+ date+"/")
        if os.path.exists("plots/jpgs/Migration Reruns "+ date+"/") == True:
            os.rmdir("plots/jpgs/Migration Reruns "+ date+"/")
        if os.path.exists('results/Migration Reruns '+date+".csv") == True:
            os.remove('results/Migration Reruns '+date+".csv")
       
        # 6.2: Open the CSV file to save results, write the columns, and close it.
        txt_file = open("results/Migration Reruns "+date+".csv", "a")
        ...
        os.makedirs("save/Migration Reruns "+ date+"/")
        os.makedirs("plots/jpgs/Migration Reruns "+ date+"/")
        os.makedirs("plots/pdfs/Migration Reruns "+ date+"/")

        # 6.3: Print results in the terminal and write to the CSV by calling run_migration_edge.
        for i in range(len(systems_to_rerun)):
            ...
            if i>=1:
                print("\n\n")
            # 6.3.3: Print run index.
            print("Run "+str(i)+":")
            # 6.3.4: Extract initial conditions.
            if i == 0:
                bin_file = "save/Migration Runs 08-02-2024-08-05-2024/4_inner_edge_1.1e+02_taua_2.7e+06_4.bin"
            if i == 1:
                bin_file =  "save/Migration Runs 08-02-2024-08-05-2024/3_inner_edge_1.1e+02_taua_1.3e+05_3.bin"
            rerun_params = find_simulation_parameters(systems_to_rerun, i, bin_file)
            # 6.3.5: Initiate run_migration_edge.
            close_encounter, random_pnum = run_migration_edge(i, txt_file, rerun_params, "rerun_v10", date, integration_time="3tau_a", n_step = 10000)
            del close_encounter, random_pnum
            # 6.3.6: Close the CSV.
            txt_file.close()

runit("testhello", systems_to_rerun)